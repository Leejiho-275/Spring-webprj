...
public class ReplyApiController {
    ...
    // 댓글 목록 조회
    // ResponseEntity : 클라이언트에 응답할 때 단순 데이터만 넘기는게 아닌
    //                  각종 응답정보(상태코드, 응답헤더정보)를 함께 넘겨주는 객체
    @GetMapping("/{bno}/{page}")
    public ResponseEntity<Map<String, Object>> getList(@PathVariable Long bno, @PathVariable int page) {
    ...
    public ResponseEntity<String> create(@RequestBody Reply reply) {
        ...
        return count == 1 ? new ResponseEntity<>("regSuccess", HttpStatus.OK)
